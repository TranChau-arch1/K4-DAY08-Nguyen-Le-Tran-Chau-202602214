...
- Công thức: `score = W_U·U + W_A·A + W_D·D` (trọng số mặc định: $W_U = 0.5, W_A = 0.3, W_D = 0.2$).
  ...
  - **`A` (Ambiguity):** Tỷ lệ số box mập mờ ($0.15 \le conf < 0.50$), chuẩn hóa chia cho giá trị lớn nhất trong pool. Frame có nhiều box mập mờ thể hiện toàn cảnh đang gây rối loạn cho bộ phát hiện.
...
- **Biến động AP50:**
  - AP50 giảm mạnh từ $0.771$ xuống $0.371$, tức $\Delta\text{AP50} = -0.400$ so với khởi đầu lạnh.
- **Biến động số đo theo nhóm xe:**
  ...
  - *Mặt tiêu cực:* Recall sụt giảm thảm hại từ $0.489$ xuống còn $0.097$ ($FN = 364$). Trong đó:
    ...
    - Xe nhỏ: Recall rơi thẳng về $0.000$ (mô hình không phát hiện được bất kỳ chiếc xe nhỏ nào ở ngưỡng $conf \ge 0.25$).
...
- *Kết quả mô hình sau train:* Dù con người đã nỗ lực bổ sung 182 box bị bỏ sót vào tập train, mô hình sau fine-tune lại học theo hướng triệt tiêu box giả cực đoan, dẫn đến việc mất hẳn độ nhạy với xe nhỏ trên tập test ($R_{small} = 0.0$).
...
- *Ca 1 — Frame ở phân đoạn đầu video (`frame_0020.jpg` tại t = 8.0s):*
  - Điểm mạnh: Lấp khoảng trống dữ liệu ở đầu video, mật độ 24 box vừa phải giúp giảm áp lực công gán nhãn, cách xa các frame đã gán ở vòng 1 ($> 30\text{s}$) nên hoàn toàn không có nguy cơ gần trùng.
- *Ca 2 — Frame chứa phương tiện cỡ lớn ở làn giữa (`frame_0218.jpg` tại t = 87.2s):*
  - Giúp củng cố lại đặc trưng của các xe tải/xe khách cỡ lớn đang bị suy giảm Recall ($R_{large}$ giảm còn $0.293$). Khoảng cách thời gian cách frame gần nhất (`frame_0227.jpg` ở 90.8s) là $3.6\text{s} > \text{MIN\_GAP\_S}$, đảm bảo an toàn về độ đa dạng bối cảnh.
